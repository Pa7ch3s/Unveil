...
1. **Get the JAR** — Download `unveil-burp-0.7.6.jar` from [GitHub Releases](https://github.com/Pa7ch3s/Unveil/releases) (if published), or **on another laptop**: clone the repo, then build:
   ```bash
   git clone https://github.com/Pa7ch3s/Unveil.git && cd Unveil/unveil-burp
   ./gradlew jar
   ```
   JAR output: `build/libs/unveil-burp-0.7.6.jar`. Load that file in Burp (Extensions → Add → Java).
...
The JAR is written to `build/libs/unveil-burp-0.7.6.jar`.

## Load in Burp

1. Open Burp Suite (2023.8+ for Montoya API).
2. **Extensions** → **Installed** → **Add** → **Extension type: Java**.
3. Select `build/libs/unveil-burp-0.7.6.jar`.
