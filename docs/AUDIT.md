...
- **Version string:** **Fixed.** Single source: `pyproject.toml` version; CLI reads via `importlib.metadata.version("unveil")` (fallback `0.9.0`). Banner and `--version` are dynamic.
